...
from model_multiview import ALIGN
...
def main():
    ...
    for data_seed in range(1, args.test_time + 1):
        ...
        optimizer = torch.optim.Adam(
            itertools.chain(
                align.autoencoder1.parameters(), align.autoencoder2.parameters(),
                align.autoencoder3.parameters(),
                align.a2b_gen.parameters(), align.b2a_gen.parameters(),
                align.a2c_gen.parameters(), align.c2a_gen.parameters(),
                align.b2c_gen.parameters(), align.c2b_gen.parameters()
            ),
            lr=config['training']['lr']
        )

        logger.info(align.autoencoder1)
        logger.info(align.a2b_gen)
        logger.info(optimizer)

        align.autoencoder1.to(device), align.autoencoder2.to(device), align.autoencoder3.to(device)
        align.a2b_gen.to(device), align.b2a_gen.to(device)
        align.b2c_gen.to(device), align.c2b_gen.to(device)
        align.a2c_gen.to(device), align.c2a_gen.to(device)

        if config['type'] == 'CG':
            acc, precision, f_measure = align.train_completegraph_supervised(config, logger, accumulated_metrics,
