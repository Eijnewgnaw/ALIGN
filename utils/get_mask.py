...
def get_mask(view_num, alldata_len, missing_rate):
    ...
    if alldata_len != 0:
        ...
        while error >= 0.005:
            ...
            np_int = np.int32
            matrix_iter = (randint(0, 100, size=(alldata_len, view_num)) < int(ratio * 100)).astype(np_int)
            a = np.sum(((matrix_iter + view_preserve) > 1).astype(np_int))
            one_num_iter = one_num / (1 - a / one_num)
            ratio = one_num_iter / (view_num * alldata_len)
            matrix_iter = (randint(0, 100, size=(alldata_len, view_num)) < int(ratio * 100)).astype(np_int)
            matrix = ((matrix_iter + view_preserve) > 0).astype(np_int)
